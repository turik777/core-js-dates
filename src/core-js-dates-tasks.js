...
function getNextFriday(date) {
  ...
  const daysToFriday =
    dateNew.getDay() === 5 ? 7 : (7 + 5 - dateNew.getDay()) % 7;
  dateNew.setDate(dateNew.getDate() + daysToFriday);
  return dateNew;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (current <= end) {
    for (let i = 0; i < countWorkDays && current <= end; i += 1) {
      schedule.push(current.toLocaleDateString('uk').replaceAll('.', '-'));
